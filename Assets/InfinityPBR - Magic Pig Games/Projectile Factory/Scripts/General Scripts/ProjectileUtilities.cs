...
namespace MagicPigGames.ProjectileFactory
{
    ...
    public static class ProjectileUtilities
    {
        ...
        public static void SetLayer(GameObject obj, int newLayer, bool includeChildren = true)
        {
            ...
            Debug.Log($"Setting layer of {obj.name} to {newLayer} from {obj.layer}");
            obj.layer = MaskToIndex(newLayer);
        ...
        static int MaskToIndex(LayerMask mask)
        {
            // Assumes only one layer is selected. 
            int m = mask.value;
            if (m == 0) return 0;
            return (int)Mathf.Log(m, 2);  // e.g. Log(128,2)=7
        }
